...
import numpy as np
R=Path(__file__).resolve().parents[1];O=R/'outputs/fragment_repair_guard_20260920'
...
def main():
    ...
    c=json.loads((R/'config/fragment_repair_guard_20260920.json').read_text())
    ...
    assert raw_count==run['episodes']==30720
    ...
    result=dict(run=run,aggregate=aggregate,iterations=iterations,bootstrap_replicates=c['bootstrap_replicates'],
        bootstrap_seed=c['bootstrap_seed'],lower_quantile=c['lower_quantile'],family_claims=8,
    ...
    log=(R/'tmp/fragment_guard_validation_20260920.log').read_text()
...
- {run['episodes']:,} episodes,96 scenario cells,64 paired development seed units,
 8 workers,{run['seconds']:.3f}s. Seeds9700200--9700263; bootstrap9789997.
...
'''
    rp=R/'reports/FRAGMENT_REPAIR_GUARD_RESULTS_20260920.md'
    with rp.open('x') as f:f.write(report)
    files=[rp,O/'results.json',O/'comparison.csv',O/'seed_means.json',R/'tmp/fragment_guard_validation_20260920.log',R/'tmp/fragment_repair_guard_20260920.log']
