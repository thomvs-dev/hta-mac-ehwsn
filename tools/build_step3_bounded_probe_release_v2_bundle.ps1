...
python -B (Join-Path $PSScriptRoot "generate_step3_bounded_probe_colab_release_v2.py") --bundle-sha256 $hash --output $notebook
if ($LASTEXITCODE -ne 0) { throw "Release-v2 notebook generation failed" }
Write-Output "RELEASE_V2_BUNDLE=$zip"
Write-Output "RELEASE_V2_SHA256=$hash"
Write-Output "RELEASE_V2_NOTEBOOK=$notebook"
