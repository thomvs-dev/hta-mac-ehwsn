...
def _slot_candidates(env, caps, action, queue_before, ages_before, info, energy_before):
    ...
    for node in current_members:
        distance = float(np.linalg.norm(env.base.positions[node] - env.base.positions[current_ch]))
                ...
                for env in environments:
                    ...
                    while not done:
                        ...
                        candidates = _slot_candidates(
                            env, caps, action, queue_before, ages_before, info, energy_before
                        )
                        ...
                        feasible_costs = sorted(
                            env.base.radio.tx(
                                ...
                                float(np.linalg.norm(env.base.positions[row["node"]] - env.base.positions[int(info["target_ch"])])),
                            )
