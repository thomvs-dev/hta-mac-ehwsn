import pytest
from core.fragment_repair_guard import (fragment,parse_fragment,ack_packet,parse_ack,
...
def test_deadline_guard_abstains_from_incomplete_last_frame_plan():
    from core.fragment_repair_guard import CONTROL_UJ
    t=controlled()
    costs=plan_cost('guard',tuple(range(N)),tuple(range(N)))
    capacity=CONTROL_UJ+60*6+min(costs[:2])-100
    r=evaluate(t,'guard',capacity_uj=capacity,harvest_uj=0,ttl=1,loss='clean',ack_loss=0,trace=True)
    assert r['deadline_abstentions']>0
    assert not r['records'][0]['events']
