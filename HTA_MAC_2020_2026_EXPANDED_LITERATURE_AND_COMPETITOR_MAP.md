...
| **A8** | Dutta, Bhuyan, and Biswas, “Contextual Deep Reinforcement Learning for Flow and Energy Management in Wireless Sensor and IoT Networks,” *IEEE Transactions on Green Communications and Networking* (2024), DOI [10.1109/TGCN.2024.3358230](https://doi.org/10.1109/TGCN.2024.3358230) | Resource-constrained arbitrary mesh WSN/IoT; no harvest/solar model in the author manuscript | Three tiers: per-node MAB chooses one collision-free slot; per-flow CDQL chooses discretized transmit probability from queue-gradient state and traffic-rate context; per-node MAB chooses sleep/listen in other slots and explicitly accounts for idle listening | **Very important target-journal competitor.** It invalidates broad novelty for learned joint slot/sleep or idle-listening-aware scheduling. HTA-MAC differs through clustered hybrid-HMM energy features, shared branching multi-slot counts, and hard cluster-budget projection. |
...
| **A12** | Dutta, Bhuyan, and Biswas, “Cooperative Reinforcement Learning for Energy Management in Multi-Hop Networks With Energy Harvesting,” *IEEE TGCN* 9(4) (2025), DOI [10.1109/TGCN.2025.3544073](https://doi.org/10.1109/TGCN.2025.3544073) | Multi-hop EH sensor/IoT network; two-state Markov solar generator | Two tabular Q-learning agents per node: quantized harvested-energy state → transmission probability; transmission-policy state → transceiver-on probability; shared reward and neighbor learning-confidence sharing | **Closest target-journal harvest-aware learned transmit/sleep competitor.** It invalidates broad novelty for Markov-solar-conditioned RL scheduling. HTA-MAC differs through clustered hybrid solar/thermal transition features, shared Branching C51, discrete multi-slot counts, and hard cluster-budget projection. |
