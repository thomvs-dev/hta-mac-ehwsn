...
POLICIES=('whole','selective','block','prefix','guard')
...
def evaluate(tape,policy,*,capacity_uj=16000,harvest_uj=300,head_change=False,
    ...
    observed_head=None;counts=dict(data_fragments=0,ack_frames=0,duplicate_fragments=0,
        deadline_abstentions=0,grant_frames=0,control_uj=0,data_uj=0,background_uj=0,member_uj=0,forward_uj=0,
    ...
    for t in range(horizon):
        ...
        if all(ready) and alive[0] and alive[head]:
            ...
            for p,_ in sorted(reported,key=lambda z:(z[1],z[0])):
                ...
                member,forward=plan
                if policy=='guard' and t+1>=p+ttl:
                    # Head-local cache and received sink ACK only: do not infer
                    # hidden receiver state or the next channel outcome.
                    can_supply=(cached|mask(member)|known_sink)==FULL
                    can_complete=(mask(forward)|known_sink)==FULL
                    if not(can_supply and can_complete):
                        counts['deadline_abstentions']+=1
                        continue
                cs,ch,max_duration=plan_cost(policy,member,forward)
